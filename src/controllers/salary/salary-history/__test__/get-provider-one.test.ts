...
describe('Get Provider Salary History One', () => {
    it('should return salary history successfully', async () => {
        const response = await request
            .get(`/api/v1/salaries/history/${salaryHistory.id}`)
            .set('id', users[0].id);
    ...
    it('should return 404 if salary history is not found', async () => {
        ...
        const response = await request
            .get(`/api/v1/salaries/history/${nonExistentSalaryHistoryId}`)
            .set('id', users[0].id);
    ...
    it('should return 422 if id validation failed', async () => {
        ...
        const response = await request
            .get(`/api/v1/salaries/history/${invalidId}`)
            .set('id', users[0].id);
