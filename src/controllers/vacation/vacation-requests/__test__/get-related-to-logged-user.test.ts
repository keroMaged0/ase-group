...
describe('Get Related to Logged User Controller', () => {
  it('should get a list of vacation requests related to the logged user', async () => {
    ...
    const response = await request.get('/api/v1/vacations/requests/me').set('id', users[0].id);
  ...
  it('should get a single vacation request related to the logged user by ID', async () => {
    ...
    const response = await request
      .get(`/api/v1/vacations/requests/me/${vacationRequest.id}`)
      .set('id', users[0].id);
  ...
  it('should return 404 not found if vacation request related to the logged user does not exist', async () => {
    const response = await request
      .get(`/api/v1/vacations/requests/me/${v4()}`)
      .set('id', users[0].id);
