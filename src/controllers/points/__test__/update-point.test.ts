...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.update_point });
...
describe('Update point', () => {
  it('should update point successfully', async () => {
    ...
    const updatedPoint = await dataSource.getRepository(Point).findOne({
      where: { name: 'Updated Test point' },
    });
    expect(updatedPoint).toBeTruthy();
    expect(updatedPoint?.points).toBe(10);
  });

  it('should fail to update point with invalid id', async () => {
    const response = await request
      .put('/api/v1/points/invalid-uuid')
      .send({
        name: 'Updated Test point',
        description: 'Sample description',
        points: 10,
