...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.get_point });
...
describe('Get One point', () => {
  it('should return a point with the given id', async () => {
    const response = await request.get(`/api/v1/points/${point.id}`).set('id', users[0].id);

    expect(response.status).toBe(200);
    expect(response.body.success).toBe(true);
  });

  it('should return not found if the point does not exist', async () => {
    const response = await request.get(`/api/v1/points/1`);
