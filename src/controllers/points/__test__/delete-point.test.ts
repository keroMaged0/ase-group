...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.remove_point });
  ...
  point = await dataSource.getRepository(Point).save({
    created_by: { id: users[0].id },
...
describe('Delete point', () => {
  it('should delete point successfully', async () => {
    const response = await request.delete(`/api/v1/points/${point.id}`).set('id', users[0].id);

    expect(response.status).toBe(200);

    const deletedPoint = await dataSource.getRepository(Point).findOneBy({
      name: 'Sample Point Name',
    });
  ...
  it('should return 404 if point not found', async () => {
    const response = await request.delete(`/api/v1/points/${v4()}`).set('id', users[0].id);

    expect(response.status).toBe(404);
  });

  it('should fail to delete point with invalid id', async () => {
    const response = await request.delete('/api/v1/points/invalid-uuid').set('id', users[0].id);
