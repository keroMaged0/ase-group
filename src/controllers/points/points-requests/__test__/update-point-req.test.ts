...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.update_point_request });
...
describe('Update point request', () => {
  it('should update point request successfully', async () => {
    const response = await request
      .put(`/api/v1/points-request/${pointReq.id}`)
      .send({
        title: 'Updated Test point request',
        description: 'Sample description for point request',
        status: 1,
      })
      .set('id', users[0].id);
    ...
    const updatedPointReq = await dataSource.getRepository(pointsRequest).findOne({
      where: {
        status: 1,
      },
    });
    expect(updatedPointReq).toBeTruthy();
    expect(updatedPointReq?.title).toBe('Updated Test point request');
  });

  it('should fail to update point request with invalid id', async () => {
    const response = await request
      .put('/api/v1/points-request/invalid-uuid')
      .send({
        title: 'Updated Test point request',
        description: 'Sample description for point request',
        status: 1,
