...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.remove_target });
  ...
  target = await dataSource.getRepository(Target).save({
    ...
    target_type: TargetType.Quarterly,
    target_quantity: 20,
    target_amount: 2000,
...
describe('Delete target', () => {
  it('should delete target successfully', async () => {
    ...
    const deletedTarget = await dataSource.getRepository(Target).findOne({
      where: { id: target.id },
    });

    expect(deletedTarget?.is_deleted).toBe(true);
  });

  it('should return 404 if target not found', async () => {
    const response = await request.delete(`/api/v1/targets/${v4()}`).set('id', users[0].id);

    expect(response.status).toBe(404);
  });

  it('should fail to delete target with invalid id', async () => {
    const response = await request.delete('/api/v1/targets/invalid-uuid').set('id', users[0].id);
