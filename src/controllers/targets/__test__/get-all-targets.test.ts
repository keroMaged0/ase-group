...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.get_all_target });
...
describe('Get all targets', () => {
  it('should get all targets successfully', async () => {
    const response = await request.get('/api/v1/targets').set('id', users[0].id);
  ...
  it('should not access target if user has no permission', async () => {
