...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.get_one_target });
...
describe('Get One point', () => {
  it('should return a target with the given id', async () => {
    const response = await request.get(`/api/v1/targets/${target.id}`).set('id', users[0].id);

    expect(response.status).toBe(200);
    expect(response.body.success).toBe(true);
  });

  it('should return not found if the target does not exist', async () => {
    const response = await request.get(`/api/v1/targets/1`);
