...
beforeEach(async () => {
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.remove_product });
...
describe('Delete product', () => {
  it('should delete poduct successfully', async () => {
  ...
  it('should fail to delete product with invalid id', async () => {
    const response = await request.delete('/api/v1/products/invalid-uuid').set('id', users[0].id);
