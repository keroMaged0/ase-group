...
beforeEach(async () => {
  ...
  const permissions = await dataSource
    ...
    .save({ key: PERMISSIONS.article_action });
...
describe('POST /api/v1/articles/:id/toggle-action', () => {
  it('should add the article action successfully', async () => {
    const response = await request(app)
      .post(`/api/v1/articles/${testArticleId}/toggle-action`)
      .set('id', testUserId);
    ...
    const action = await actionRepo.findOne({
      where: { article_id: testArticleId, user_id: testUserId },
    });
  ...
  it('should remove action successfully', async () => {
    ...
    const action = await dataSource.getRepository(ArticleAction).findOne({
      where: { article_id: testArticleId, user_id: testUserId },
    });
  ...
  it('should return 404 if the article does not exist', async () => {
    ...
    const response = await request(app)
      .post(`/api/v1/articles/${nonExistentArticleId}/toggle-action`)
      .set('id', testUserId);
  ...
  it('should return 422 if the article id is not valid', async () => {
    ...
    const response = await request(app)
      .post(`/api/v1/articles/${notValidId}/toggle-action`)
      .set('id', testUserId);
