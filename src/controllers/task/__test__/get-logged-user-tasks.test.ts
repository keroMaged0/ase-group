...
beforeEach(async () => {
    const permissions = await dataSource
        ...
        .save({ key: PERMISSIONS.get_logged_user_tasks });
...
describe('Get Logged User Tasks', () => {
    it('should fetch all tasks for the logged user', async () => {
        const response = await request
            .get('/api/v1/tasks/logged-user')
            .set('id', users[0].id);
    ...
    it('should fetch tasks with specific status for the logged user', async () => {
        const response = await request
            .get('/api/v1/tasks/logged-user')
    ...
    it('should fetch tasks within a date range for the logged user', async () => {
    ...
    it('should return 403 if user has no permission to view tasks', async () => {
